...
const DeliveryForwardPage = () => {
  ...
  const {
    ...
    handleCreateNew,
    saveExcelData,
  } = useDeliveryForwardContext();

  // State for modal visibility and form inputs
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newDeliveryForward, setNewDeliveryForward] = useState({
    name: "",
    description: "",
  ...
  const handleSaveExcelData = async () => {
    try {
      const updatedDeliveryForward = await saveExcelData(
        selectedExcelId,
        excelTableData,
        excelFileName
      );
      ...
      setDeliveryForwardData((prev) =>
        prev.map((deliveryForward) =>
          deliveryForward.id === updatedDeliveryForward.id
            ? { ...deliveryForward, ...updatedDeliveryForward }
            : deliveryForward
        )
      );
  ...
  const filteredData = deliveryForwardData.filter((row) => {
    ...
    <div className="p-5">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-3xl text-white font-bold">
          Delivery Forward Status
        </h2>
        <button
          className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700"
          onClick={() => setIsModalOpen(true)}
        >
          + New Delivery Forward
        </button>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white w-3/5 max-h-[80vh] p-6 rounded-lg overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">New Delivery Forward</h2>
              <button
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  name="name"
                  value={newDeliveryForward.name}
                  onChange={(e) =>
                    setNewDeliveryForward({
                      ...newDeliveryForward,
                      name: e.target.value,
                    })
                  }
              ...
              <div>
                ...
                <input
                  type="text"
                  name="createdBy"
                  value={newDeliveryForward.createdBy}
                  onChange={(e) =>
                    setNewDeliveryForward({
                      ...newDeliveryForward,
              ...
              <div className="col-span-2">
                ...
                <textarea
                  name="description"
                  value={newDeliveryForward.description}
                  onChange={(e) =>
                    setNewDeliveryForward({
                      ...newDeliveryForward,
              ...
              <div>
                ...
                <select
                  name="status"
                  value={newDeliveryForward.status}
                  onChange={(e) =>
                    setNewDeliveryForward({
                      ...newDeliveryForward,
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button
                ...
                onClick={async () => {
                  try {
                    await handleCreateNew(newDeliveryForward);
...
export default DeliveryForwardPage;
